...
repo's manifests and VERIFY each with the cheapest check that proves it
resolves (runbook step 1): a config-parse check for linters
(`verified: config`), `--version` for test runners and hooks
(`verified: liveness`) — NOT a full analysis run. Return,
compactly: (a) proposed commands with evidence + verification tier,
...
**Command hygiene (critical during verification).** Run each verification
check PLAINLY — e.g. `bundle exec rubocop --list-target-files`, not
